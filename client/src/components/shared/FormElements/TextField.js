...
const TextField = ({
    ...
    const { returnFormField, inputChangeHandler, errorIds } = useForm()
      ...
      const formatValue = (value, pattern) => {
        ...
        return [...pattern].map((patternCharacter, index) => {
      ...
      const handleChange = (event) => {
        ...
        if (didDelete) {
          const patternCharacterDeleted = !isUserCharacter([...value][cursorPosition]);
    ...
    return (
        <div key={label} className={`${error ? 'mb-2' : 'mb-8'} flex-1 ${wrapperClasses}`}>
            ...
            />
            {error && <p className='text-red-600 text-sm mt-1'>{error.message.replace('{label}', label)}</p>}
        </div>
    )
}
