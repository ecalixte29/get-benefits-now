        ...
        try {
            ...
            if (contactDoc.dependents) {
                contactDoc.dependents.forEach((dependent, index) => {
                    ...
                    dependentsDetails[GHL_CUSTOM_FIELDS[`${keyPrefix}_full_name`]] = `${dependent.first_name} ${dependent.last_name}`;
                    dependentsDetails[GHL_CUSTOM_FIELDS[`${keyPrefix}_dob`]] = formatDate(dependent.dob);
                    dependentsDetails[GHL_CUSTOM_FIELDS[`${keyPrefix}_ssn`]] = dependent.social_security_number;
                    if (index >= 1 && index <= 2) {
                        dependentsDetails[GHL_CUSTOM_FIELDS[`contact.do_you_have_a_${index + 1}nd_dependent`]] = "Yes";
                    }
                });
            }
            ...
            const contact = {
                ...
                customField: {
                    ...
                    [GHL_CUSTOM_FIELDS["contact.are_you_a_us_national_"]]: contactDoc.details.us_national === 'true' ? 'Yes' : 'No',
                    [GHL_CUSTOM_FIELDS["contact.are_you_a_tobacco_user_"]]: contactDoc.details.uses_tobacco === 'true' ? 'Yes' : 'No',
                    [GHL_CUSTOM_FIELDS["contact.are_you_on_medicaid_or_medicare"]]: contactDoc.details.current_insurance === 'true' ? 'Yes' : 'No',
                    ...dependentsDetails,
                }
            };
            ...
            if (contactDoc.spouse_details) {
                const spouseCustomFields = {
                    ...
                    [GHL_CUSTOM_FIELDS["contact.spouse_full_name"]]: `${contactDoc.spouse_details.first_name} ${contactDoc.spouse_details.last_name}`,
                    [GHL_CUSTOM_FIELDS["contact.spouse_date_of_birth"]]: formatDate(contactDoc.spouse_details.dob),
                    [GHL_CUSTOM_FIELDS["contact.spouse_email"]]: contactDoc.spouse_details.email,
                    [GHL_CUSTOM_FIELDS["contact.spouse_ssn"]]: contactDoc.spouse_details.social_security_number,
                    [GHL_CUSTOM_FIELDS["contact.spouse_phone"]]: contactDoc.spouse_details.phone,
                    [GHL_CUSTOM_FIELDS["contact.do_your_partner_uses_tobacco"]]: contactDoc.spouse_details.uses_tobacco === 'true' ? 'Yes' : 'No'
                };
                contact.customField = { ...contact.customField, ...spouseCustomFields };
            }
            ...
            if (res.status === 200) console.log('Contact published on GHL successfully 🎉');
